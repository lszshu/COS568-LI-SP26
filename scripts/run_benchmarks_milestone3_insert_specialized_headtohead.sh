...
echo "Executing dataset=${DATASET}, insert-ratio=${INSERT_RATIO}, index=DynamicPGM"
"$BENCHMARK" "$DATA_FILE" "$OPS_FILE" --through --csv --only DynamicPGM -r 3

echo "Executing dataset=${DATASET}, insert-ratio=${INSERT_RATIO}, index=HybridPGMLIPPIncremental, params=2097152,1,0,1,0,0,0,0"
"$BENCHMARK" "$DATA_FILE" "$OPS_FILE" --through --csv --only HybridPGMLIPPIncremental -r 3 \
    --params 2097152,1,0,1,0,0,0,0

echo "Executing dataset=${DATASET}, insert-ratio=${INSERT_RATIO}, index=HybridPGMLIPPInsertSpecialized"
"$BENCHMARK" "$DATA_FILE" "$OPS_FILE" --through --csv --only HybridPGMLIPPInsertSpecialized -r 3
