...
def variant_string(row: pd.Series) -> str:
    return ":".join(
        str(value)
        for value in [row.get(col, "") for col in VARIANT_COLUMNS]
...
def load_summary(run_root: Path, thread_count: int) -> pd.DataFrame:
    ...
    for dataset_key in DATASETS:
        ...
        for insert_ratio, workload_label in WORKLOADS:
            ...
            if lipp is None or mt_hybrid is None:
                continue
            ...
            rows.append(
                {
                    ...
                    "single_lipp_variant": variant_string(lipp),
                    ...
                    - lipp["median_mops"],
                    "mt_hybrid_speedup_vs_single_lipp": mt_hybrid["avg_mops"] / lipp["avg_mops"],
                    "mt_hybrid_median_speedup_vs_single_lipp": mt_hybrid["median_mops"]
                    / lipp["median_mops"],
                }
            )
    if not rows:
...
def plot_summary(summary: pd.DataFrame, output_dir: Path):
    ...
    for row_idx, (_, workload_label) in enumerate(WORKLOADS):
        for col_idx, dataset_key in enumerate(DATASETS):
            ...
            row = subset.iloc[0]
            labels = ["LIPP-1T", "Hybrid-1T", "Hybrid-MT"]
            values = [
                row["single_lipp_mops"],
                row["single_hybrid_mops"] if pd.notna(row["single_hybrid_mops"]) else 0,
                row["mt_hybrid_mops"],
            ]
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "milestone3_multithread_fast_throughput.png", dpi=300)
...
def main():
    ...
    summary.to_csv(analysis_dir / "milestone3_multithread_fast_summary.csv", index=False)
