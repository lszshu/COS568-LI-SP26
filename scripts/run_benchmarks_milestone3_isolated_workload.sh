...
declare -a HYBRID_PARAMS=()
if [ "$INSERT_RATIO" = "0.100000" ]; then
    HYBRID_PARAMS=(
        "131072,256,0,0,0,0,0,1"
        "262144,1,0,1,1,6,0,0"
        "262144,1,0,1,1,0,0,0"
    )
else
    HYBRID_PARAMS=(
        "262144,2048,0,1,0,0,0,0"
        "1048576,1,0,1,0,0,0,0"
        "2097152,1,0,1,0,0,0,0"
        "2097152,1,0,0,0,0,0,0"
    )
fi

for PARAMS in "${HYBRID_PARAMS[@]}"
do
    echo "Executing dataset=${DATASET}, insert-ratio=${INSERT_RATIO}, index=HybridPGMLIPPIncremental, params=${PARAMS}"
