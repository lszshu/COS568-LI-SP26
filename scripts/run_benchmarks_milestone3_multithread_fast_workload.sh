...
cd "$MULTI_DIR"
echo "Executing multithread dataset=${DATASET}, insert-ratio=${INSERT_RATIO}, threads=${THREAD_COUNT}, index=HybridPGMLIPPConcurrentWorkloadAware"
"$BENCHMARK_COPY" "$DATA_FILE" "$MT_OPS_FILE" --through --csv --only HybridPGMLIPPConcurrentWorkloadAware --threads "$THREAD_COUNT" -r 3
